...
def _fetch_stooq(ticker: str, start: str) -> pd.Series:
    ...
    Stooq returns its refusals as plain text with HTTP 200 — a body of
    "Exceeded the daily hits limit" parses as an empty frame, which is why
    a limit block used to surface as a generic 'no data' error.
    """
...
def fetch_one(ticker: str, start: str, source: str, token: str = "") -> pd.Series:
    """Cached per ticker, not per universe.

    Caching the whole universe meant any change to the selection — switching
    benchmark, deselecting a basket — produced a new cache key and refetched
    all ~51 tickers. That's what burns through a daily request allowance in
    a few clicks. Keyed per ticker, a benchmark switch costs one request.
    """
    if source == "Tiingo":
        return _fetch_tiingo(ticker, start, token)
    return _fetch_stooq(ticker, start)


def fetch_prices(tickers: tuple, start: str, source: str, token: str = ""):
    """Returns (prices, failed, rate_limited)."""
    collected: dict = {}
    failed: list = []
    rate_limited = False
    ...
    for ticker in tickers:
        try:
            s = fetch_one(ticker, start, source, token)
        except RuntimeError:
            rate_limited = True
            failed.append(ticker)
            continue           # no point hammering a source that's blocking us
        except Exception:
            failed.append(ticker)
            continue
...
if px.empty or (bench_ticker and bench_ticker not in px.columns):
    if rate_limited and source == "Stooq":
        st.error(
            "Stooq is refusing requests — you've hit its daily limit for this "
            "IP. It resets on their schedule (roughly daily), and there's no "
            "way to raise it. Switch the source to Tiingo in the sidebar."
        )
    elif rate_limited:
        st.error(
            "Tiingo's hourly request limit is exhausted. Wait for the hour to "
            "roll over, or deselect a few baskets to fit inside 50 requests."
        )
    else:
        st.error(
            f"No data came back from {source}. Check your connection, or try "
